 ...
 */
/*armin*/
void savexy(stamp_struct* stamps, int nStamps, long xmin, long ymin,
            int regioncounter) {
  int sscnt, istamp;
  ...
  snprintf(xyfilenameskipped, sizeof(xyfilenameskipped), "%s.skipped",
           xyfilename);
  ...
  for (istamp = 0; istamp < nStamps; istamp++) {
    for (sscnt = 0; sscnt < stamps[istamp].nss; sscnt++) {
      /*fprintf(xyfileall,        " %4ld %4ld\n", stamps[istamp].xss[sscnt] +
       * xmin, stamps[istamp].yss[sscnt] + ymin);*/
 ...
 *   - stamps[i].krefArea: fwKSStamp² double array for the reference-image
 * patch.
 *   - stamps[i].mat     : nC × nC double array for the per-stamp
...
int allocateStamps(stamp_struct* stamps, int nStamps) {
  ...
  if (stamps) {
    for (stampIdx = 0; stampIdx < nStamps; stampIdx++) {
      /* Allocate contiguous vector array: much more efficient than nested
       * malloc */
      stamps[stampIdx].vectors =
          alloc_vector_array(nCompKer + nbgVectors, fwKSStamp * fwKSStamp);

      /* Reference area for kernel fitting */
      stamps[stampIdx].krefArea =
          (double*)xcalloc((size_t)fwKSStamp * fwKSStamp, sizeof(double));
      ...
      stamps[stampIdx].x0 = stamps[stampIdx].y0 = stamps[stampIdx].x =
          stamps[stampIdx].y = 0;
      stamps[stampIdx].nss = stamps[stampIdx].sscnt = 0;
      stamps[stampIdx].nx = stamps[stampIdx].ny = 0;
      stamps[stampIdx].sum = stamps[stampIdx].mean = stamps[stampIdx].median =
          0;
 ...
 *
 * @details For each of the template and image stamp arrays (ctStamps,
 * ciStamps), if no substamps have been found yet, calls cutStamp() to extract
 * the stamp pixel data and getStampStats3() to compute sky background
 * statistics (mode, FWHM used as sky sigma).
 *
 ...
 *
 * @param sXMin      Left pixel bound of the stamp region in global image
 * coords.
...
void buildStamps(int sXMin, int sXMax, int sYMin, int sYMax, int* niS, int* ntS,
  ...
  int bbitt1 = FLAG_T_BAD, bbitt2 = FLAG_T_SKIP, bbiti1 = FLAG_I_BAD,
      bbiti2 = FLAG_I_SKIP;
  float* refArea = NULL;
  double check;

  LOG_DEBUG("Stamp in region: %d:%d,%d:%d", sXMin, sXMax, sYMin, sYMax);
  ...
  if (!(strncmp(forceConvolve, "i", 1) == 0)) {
    if (ctStamps[*ntS].nss == 0) {
      ...
      if (!(getStampStats3(refArea, ctStamps[*ntS].x0, ctStamps[*ntS].y0, sPixX,
                           sPixY, &ctStamps[*ntS].sum, &ctStamps[*ntS].mean,
                           &ctStamps[*ntS].median, &ctStamps[*ntS].mode,
                           &ctStamps[*ntS].sd, &ctStamps[*ntS].fwhm,
                           &ctStamps[*ntS].lfwhm, 0x0, 0xffff, 3))) {
        /* pointless */
        LOG_PROGRESS("Template xs : %4i ys : %4i  (sky,dsky = %.1f,%.1f)",
                     ctStamps[*ntS].x, ctStamps[*ntS].y, ctStamps[*ntS].mode,
                     ctStamps[*ntS].fwhm);
      }
      free(refArea);
    }
  ...
  if (!(strncmp(forceConvolve, "t", 1) == 0)) {
    if (ciStamps[*niS].nss == 0) {
      ...
      if (!(getStampStats3(refArea, ciStamps[*niS].x0, ciStamps[*niS].y0, sPixX,
                           sPixY, &ciStamps[*niS].sum, &ciStamps[*niS].mean,
                           &ciStamps[*niS].median, &ciStamps[*niS].mode,
                           &ciStamps[*niS].sd, &ciStamps[*niS].fwhm,
        ...
        LOG_PROGRESS("Image xs : %4i ys : %4i  (sky,dsky = %.1f,%.1f)",
                     ciStamps[*niS].x, ciStamps[*niS].y, ciStamps[*niS].mode,
                     ciStamps[*niS].fwhm);
      }
      free(refArea);
    }
  ...
  if (!(strncmp(forceConvolve, "i", 1) == 0)) {
    ...
    if (getCenters) {
      ...
      getPsfCenters(&ctStamps[*ntS], tRData, sPixX, sPixY, tUKThresh, bbitt1,
                    bbitt2);
      LOG_PROGRESS("Template: scnt = %2i nss = %2i", ctStamps[*ntS].sscnt,
                   ctStamps[*ntS].nss);
      ...
      if (nss < nKSStamps) {
        ...
        check = checkPsfCenter(tRData, xmax - ctStamps[*ntS].x0,
                               ymax - ctStamps[*ntS].y0, sPixX, sPixY,
                               ctStamps[*ntS].x0, ctStamps[*ntS].y0, tUKThresh,
                               ctStamps[*ntS].mode, 1. / ctStamps[*ntS].fwhm, 0,
                               0, bbitt1 | bbitt2 | 0xbf, bbitt1);
        ...
        if (check != 0.) {
          ...
          for (l = ymax - hwKSStamp; l <= ymax + hwKSStamp; l++) {
            ...
            for (k = xmax - hwKSStamp; k <= xmax + hwKSStamp; k++) {
              ...
              if (nr2 >= 0 && nr2 < rPixX * rPixY) mRData[nr2] |= bbitt2;
            }
          }
  ...
  if (!(strncmp(forceConvolve, "t", 1) == 0)) {
    ...
    if (getCenters) {
      /* get potential centers for the kernel fit */
      getPsfCenters(&ciStamps[*niS], iRData, sPixX, sPixY, iUKThresh, bbiti1,
                    bbiti2);
      LOG_PROGRESS("Image: scnt = %2i nss = %2i", ciStamps[*niS].sscnt,
                   ciStamps[*niS].nss);
    } else {
      ...
      if (nss < nKSStamps) {
        ...
        check = checkPsfCenter(iRData, xmax - ciStamps[*niS].x0,
                               ymax - ciStamps[*niS].y0, sPixX, sPixY,
                               ciStamps[*niS].x0, ciStamps[*niS].y0, iUKThresh,
                               ciStamps[*niS].mode, 1. / ciStamps[*niS].fwhm, 0,
                               0, bbiti1 | bbiti2 | 0xbf, bbiti1);
        ...
        if (check != 0.) {
          ...
          for (l = ymax - hwKSStamp; l <= ymax + hwKSStamp; l++) {
            ...
            for (k = xmax - hwKSStamp; k <= xmax + hwKSStamp; k++) {
              ...
              /*if ((k > 0) && (k < sPixX) && (l > 0) && (l < sPixY))*/
              if (nr2 >= 0 && nr2 < rPixX * rPixY) mRData[nr2] |= bbiti2;
            }
          }
 ...
 * @param refArea Output buffer of size (xMax-xMin+1)*(yMax-yMin+1),
 ...
 * to be rejected.
 *
 * @param stamp  Stamp being processed; stamp->sscnt selects the active
 * substamp.
...
int cutSStamp(stamp_struct* stamp, float* iData) {
  ...
  /*stamp->krefArea = (double *)realloc(stamp->krefArea,
   * fwKSStamp*fwKSStamp*sizeof(double));*/
  ...
  if (sscnt >= nss) {
    if (verbose >= 2)
      /* This technically duplicates the verbosity check, but it'll all come out
       * in the compiler */
      LOG_DEBUG_INDENT(4,
                       "xs : %4i ys : %4i sig: %6.3f sscnt: %4i nss: %4i "
                       "******** REJECT stamp (out of substamps)",
  ...
  /*
    fprintf(stderr, "    xs : %4i ys : %4i substamp (sscnt=%d, nss=%d) xss: %4i
    yss: %4i\n", stamp->x, stamp->y, sscnt, nss, stamp->xss[sscnt],
    stamp->yss[sscnt]);
  */
  LOG_DEBUG_INDENT(4, "xss: %4i yss: %4i", stamp->xss[sscnt],
                   stamp->yss[sscnt]);

  for (substampRow = yStamp - hwKSStamp; substampRow <= yStamp + hwKSStamp;
       substampRow++) {
    y = substampRow - (yStamp - hwKSStamp);
    dy = substampRow + stamp->y0;

    for (substampCol = xStamp - hwKSStamp; substampCol <= xStamp + hwKSStamp;
         substampCol++) {
...
double checkPsfCenter(float* iData, int imax, int jmax, int xLen, int yLen,
                      int sx0, int sy0, double hiThresh, float sky,
                      float invdsky, int xbuffer, int ybuffer, int bbit,
                      int bbit1) {
  ...
  /* (5). after centroiding, re-check for fill/sat values and overlap (a little
   * inefficient) */
  ...
  for (checkRowIdx = jmax - hwKSStamp; checkRowIdx <= jmax + hwKSStamp;
       checkRowIdx++) {
    ...
    for (checkColIdx = imax - hwKSStamp; checkColIdx <= imax + hwKSStamp;
         checkColIdx++) {
      ...
      if (((dpt2 - sky) * invdsky) > kerFitThresh) dmax2 += dpt2;
    }
    if (brk == 1) break; /* exit checkRowIdx loop */
  }
 ...
 */
int getPsfCenters(stamp_struct* stamp, float* iData, int xLen, int yLen,
                  double hiThresh, int bbit1, int bbit2) {
  int stampRegionCol, stampRegionRow, centroidCol, centroidRow, nr, nr2, imax,
      jmax, xr, yr, xr2, yr2, sy0, sx0, xbuffer, ybuffer, i, j;
  ...
  */
  ...
  while (pcnt < fcnt) {
    ...
    loPsf = (loPsf > floor)
                ? loPsf
                : floor; /* last ditch attempt to get some usable pixels */

    /* (0). ignore near the edges */
    for (stampRegionRow = ybuffer; stampRegionRow < yLen - ybuffer;
         stampRegionRow++) {
      yr = stampRegionRow + sy0;

      for (stampRegionCol = xbuffer; stampRegionCol < xLen - xbuffer;
           stampRegionCol++) {
        xr = stampRegionCol + sx0;
        nr = xr + rPixX * yr;

        /* (1). pixel already included in another stamp, or exceeds hiThresh */
        if (mRData[nr] & bbit) continue;
        ...
        /* (2b). don't want low sigma point here, sometimes thats all that
         * passes first test */
        if (((dpt - sky) * invdsky) < kerFitThresh) continue;
        ...
        if (dpt > loPsf) {
          ...
          for (centroidRow = stampRegionRow - hwKSStamp;
            ...
            for (centroidCol = stampRegionCol - hwKSStamp;
              ...
              /* (4a). no fill/sat values within checked region, and not
               * overlapping other stamp */
              if (dpt2 >= hiThresh) {
                mRData[nr2] |= bbit1;
          ...
          /* (5). after centroiding, re-check for fill/sat values and overlap (a
           * little inefficient) */
          /*      zero tolerance for bad pixels! */
          ...
          dmax2 =
              checkPsfCenter(iData, imax, jmax, xLen, yLen, sx0, sy0, hiThresh,
                             sky, invdsky, xbuffer, ybuffer, bbit, bbit1);

          if (dmax2 == 0.) continue; /* continue stampRegionCol loop */
          ...
          /* globally mask out the region around this guy */
          for (centroidRow = jmax - hwKSStamp; centroidRow <= jmax + hwKSStamp;
               centroidRow++) {
            yr2 = centroidRow + sy0;

            for (centroidCol = imax - hwKSStamp;
                 centroidCol <= imax + hwKSStamp; centroidCol++) {
              xr2 = centroidCol + sx0;
              nr2 = xr2 + rPixX * yr2;

              if ((centroidCol > 0) && (centroidCol < xLen) &&
                  (centroidRow > 0) && (centroidRow < yLen))
                mRData[nr2] |= bbit2;
            }
          }
        ...
        if (brk == 2) break; /* exit x loop */
      }
      if (brk == 2) break; /* exit y loop */
    }

    if (loPsf == floor) break; /* have hit the floor, get out  */
    dfrac -= 0.2;
  }
  ...
  /* no good full stamps */
  if (pcnt == 0) {
    /* changed in v4.1.6, don't accept center pixel, could be bad, worse than
     * not having one! */
    ...
    LOG_DEBUG_INDENT(4, "Adding %d substamps found by autosearch",
    ...
    for (i = stamp->nss, j = 0; j < pcnt && i < nKSStamps; i++, j++) {
      ...
      LOG_DEBUG_INDENT(4, "#%d @ %4d,%4d,%8.1f", i, stamp->xss[i],
                       stamp->yss[i], peaks[qs[pcnt - j - 1]]);
      stamp->nss++;
    }
  }
 ...
 * @param smask    Pixels without this bit set in mRData are excluded (0
 * disables).
 */
void getNoiseStats3(float* data, float* noise, double* nnorm, int* nncount,
                    int umask, int smask) {
  ...
  for (pixelIdx = rPixX * rPixY; pixelIdx--;) {
    ...
    /*
      ...
      noiseWeightedSum, dataValue, noiseInv, maskData,
    ...
    */
    ...
    if (((umask > 0) && (!(maskData & umask))) ||
        ((smask > 0) && (maskData & smask)) || (fabs(dataValue) <= ZEROVAL))
      continue;
...
/**
 * @brief Compute robust pixel-distribution statistics (mean, median, mode,
 * sigma, FWHM) for a sub-region of an image using iterative sigma-clipping and
 *        histogram analysis.
 *
 ...
 *   are robust to outliers and provide unbiased FWHM estimates for
 * PSF-convolved noise (which is non-Gaussian due to correlation).
 *
 * @param data    Flat pixel array of the stamp region (size nPixX × nPixY).
 * @param x0Reg   X-coordinate offset of the stamp within the full region (for
 * mask lookup).
 ...
 * @param umask   Bits to exclude: pixels with (mRData[pixel] & umask) != 0 are
 * skipped.
 * @param smask   Bits to select: pixels without (mRData[pixel] & smask) set are
 * skipped.
...
int getStampStats3(float* data, int x0Reg, int y0Reg, int nPixX, int nPixY,
  ...
  */
  ...
  int binIndex, maxDensityBinIdx = 0, refinementAttempts, lowerBinIdx,
                upperBinIdx, continueRefinement;
  ...
  if (totalPixelCount < nstat) return 4;
  ...
  randomSeed =
      RNG_SEED_MAGIC; /* Numerical Recipes convention; triggers re-seeding */
  ...
  /* ====================================================================
     ...
     ===================================================================== */
  validPixelCount = 0;
  /* pull HISTOGRAM_SAMPLE_SIZE random enough values to estimate required bin
   * sizes */
  /* only do as many calls as there are points in the section! */
  /* NOTE : ignore anything with fillVal or zero */
  for (pixelIdx = 0; (pixelIdx < nstat) && (validPixelCount < totalPixelCount);
       pixelIdx++, validPixelCount++) {
    ...
    if (((umask > 0) && (!(maskData & umask))) ||
        ((smask > 0) && (maskData & smask)) || (fabs(pixelValue) <= ZEROVAL))
      pixelIdx--;
    else
  ...
  binWidth = (work[(int)(ufstat * totalPixelCount)] -
              work[(int)(mfstat * totalPixelCount)]) /
             (float)nstat; /* good estimate */
  binLowerBound = work[(int)(mfstat * totalPixelCount)] -
                  128. * binWidth; /* we use 256 bins */
  ...
  for (rowIdx = 0; rowIdx < nPixY; rowIdx++) {
    for (pixelIdx = 0; pixelIdx < nPixX; pixelIdx++) {
       ...
       * pixelValue, pixelIdx+x0Reg, rowIdx+y0Reg, maskData); */
      ...
      if (pixelValue * 0.0 != 0.0) {
        mRData[(pixelIdx + x0Reg) + (rowIdx + y0Reg) * rPixX] |=
            (FLAG_INPUT_ISBAD | FLAG_ISNAN);
        /* fprintf(stderr, "OUCH %d %d %f %d\n", pixelIdx+x0Reg, rowIdx+y0Reg,
         * pixelValue, maskData); */
        continue;
      }
  ...
  while (continueRefinement) {
    if (refinementAttempts >= MAX_SIGMA_CLIP_RETRIES) {
      ...
      /* DD fprintf(stderr, "     WARNING: 5 failed iterations in
       * getStampStats2\n"); */
      free(work);
      return 1;
    }

    for (pixelIdx = 0; pixelIdx < HISTOGRAM_NUM_BINS;
         histogram[pixelIdx++] = 0);
    ...
    for (rowIdx = 0; rowIdx < nPixY; rowIdx++) {
      for (pixelIdx = 0; pixelIdx < nPixX; pixelIdx++) {
        ...
        if ((fabs(pixelValue - (*mean)) * inverseStdev) > statSig) continue;

        binIndex = floor((pixelValue - binLowerBound) / binWidth) + 1;
        binIndex = (binIndex < 0 ? 0 : binIndex);
        binIndex = (binIndex > 255 ? 255 : binIndex);

        /* NOTE : the zero index is way overweighted since it contains
           everything below binLowerBound.
        */
        histogram[binIndex]++;
        ...
        /*
          This is the number of pixels we are working with, not totalPixelCount
          which includes any zeroed or masked pixels.
        */
        validPixelCount += 1;
      }
    }
    ...
    binSum = maxDensity = 0.;
    for (lowerBinIdx = upperBinIdx = 1; upperBinIdx < 255;
         binSum -= histogram[lowerBinIdx++]) {
      while ((binSum < validPixelCount * HISTOGRAM_PEAK_WIDTH_PCT) &&
             (upperBinIdx < 255))
    ...
    if (maxDensityBinIdx < 0 || maxDensityBinIdx > 255) maxDensityBinIdx = 0;
    ...
    for (pixelIdx = maxDensityBinIdx;
         (binSum < validPixelCount / 10.) && (pixelIdx < 255); pixelIdx++) {
    ...
    *mode = binLowerBound + binWidth * (modeBin - 1.);
    ...
    for (pixelIdx = 0, binSum = 0.; pixelIdx < maxDensityBinIdx;
         binSum += histogram[pixelIdx++]);
    binSum += histogram[maxDensityBinIdx] *
              (modeBin - maxDensityBinIdx); /*interpolate fractional bin*/
    binSum /= validPixelCount;
    modeNormalized = binSum;

    /* ====================================================================
       FWHM ESTIMATION: Find 25th–75th percentile range (robust noise σ
       estimate)
       ===================================================================== */
    /* find the region around mode containing half the "noise" points,
       e.g. assume that the mode is 50th percentile of the noise */
    lowerPercentile = validPixelCount *
                      (0.5 - HISTOGRAM_NOISE_HALF_PCT); /* 25th percentile */
    upperPercentile = validPixelCount *
                      (0.5 + HISTOGRAM_NOISE_HALF_PCT); /* 75th percentile */
    binSum = 0.;
    for (pixelIdx = 0; binSum < lowerPercentile;
         binSum += histogram[pixelIdx++]);
    lowerPercentile =
        pixelIdx - (binSum - lowerPercentile) / histogram[pixelIdx - 1];
    for (; binSum < upperPercentile; binSum += histogram[pixelIdx++]);
    upperPercentile =
        pixelIdx - (binSum - upperPercentile) / histogram[pixelIdx - 1];
  ...
  *fwhm = binWidth * (upperPercentile - lowerPercentile) / 1.35;
  ...
  /*find the median*/
  for (pixelIdx = 0, binSum = 0; binSum < validPixelCount / 2.;
       binSum += histogram[pixelIdx++]);
  *median =
      pixelIdx - (binSum - validPixelCount / 2.) / histogram[pixelIdx - 1];
 ...
 *         one point remains.
 */
int sigma_clip(float* data, int count, double* mean, double* stdev,
               int maxiter) {
  ...
  while ((newAcceptedCount != acceptedCount) && (iterationCount < maxiter)) {
    ...
    *stdev = 0;
    ...
    iterationCount += 1;
    /*fprintf(stderr, "%d %d %f %f\n", acceptedCount, newAcceptedCount, (*mean),
     * (*stdev));*/
  }
  /*fprintf(stderr, "\n");*/
  free(clipMask);
 ...
 *
 * The new contiguous allocation strategy (alloc_vector_array,
 * alloc_matrix_contiguous) means vectors and mat each require only 2 free()
 * calls total instead of N+1.
 *
 * @param stamps   Array of stamp_struct objects whose sub-arrays are to be
 * freed.
...
float* makeNoiseImage4(float* iData, float invGain, float quad) {
  ...
  if (!(nData = (float*)calloc(rPixX * rPixY, sizeof(float)))) return NULL;

  qquad = quad * quad;

  for (i = rPixX * rPixY; i--;) nData[i] = fabs(iData[i]) * invGain + qquad;
...
void getKernelInfo(char* kimage) {
  ...
  if (fits_open_file(&kPtr, kimage, 0, &status)) printError(status);
  ...
  if (!(existsTable)) {
    ...
    LOG_ERROR(
        "This image does not appear to contain a kernel table, exiting...");
    exit(1);
  }
  ...
  if (fits_close_file(kPtr, &status)) printError(status);
 ...
 * @param rXMin                Output: left pixel bound of the region
 * (0-indexed).
 * @param rXMax                Output: right pixel bound.
 * @param rYMin                Output: bottom pixel bound.
 * @param rYMax                Output: top pixel bound.
 * @param meansigSubstamps     Output: mean substamp residual sigma from the
 * fit.
 * @param scatterSubstamps     Output: scatter of substamp residuals from the
 * fit.
...
void fits_get_kernel_btbl(fitsfile* kPtr, double** kernelSol, int nRegion) {
  ...
  if (fits_read_col(kPtr, TDOUBLE, nRegion + 1, 1, 1, (nCompTotal + 1), 0,
                    *kernelSol, 0, &status))
    printError(status);

  /*
    fprintf(stderr, "OK %d, %f %f %f %f %f %f\n", nRegion, *kernelSol[0],
    *kernelSol[1], *kernelSol[192-1], *kernelSol[293-1], *kernelSol[294-1],
    *kernelSol[298-1]);
    */
 ...
 *
 * @param mData  Full-frame integer mask array (rPixX × rPixY); modified in
 * place.
    ...
    for (imageCol = 0; imageCol < rPixX; imageCol++) {
      if (mData[imageCol + rPixX * imageRow] & FLAG_INPUT_ISBAD) {
        for (horzOffset = -w2; horzOffset <= w2; horzOffset++) {
          ...
          if (ii < 0 || ii >= rPixX) continue;

          for (vertOffset = -w2; vertOffset <= w2; vertOffset++) {
            jj = imageRow + vertOffset;
            if (jj < 0 || jj >= rPixY) continue;

            mData[ii + rPixX * jj] |=
                FLAG_OK_CONV * (!(mData[ii + rPixX * jj] & FLAG_INPUT_ISBAD));
          }
        }
      }
    }
 ...
 *  - Pixel falls below the lower threshold tLThresh or iLThresh
 * (FLAG_LOW_PIXEL). After setting the initial mask, calls spreadMask() with a
 * dilation of hwKernel × kfSpreadMask1 pixels so that pixels whose convolution
 * footprint overlaps a bad pixel are correctly flagged in the output difference
 * image.
 *
...
void makeInputMask(float* tData, float* iData, int* mData) {
  ...
  for (pixelIdx = rPixX * rPixY; pixelIdx--;) {
    mData[pixelIdx] |=
        (FLAG_INPUT_ISBAD | FLAG_BAD_PIXVAL) *
        (tData[pixelIdx] == fillVal || iData[pixelIdx] == fillVal);
    mData[pixelIdx] |=
        (FLAG_INPUT_ISBAD | FLAG_SAT_PIXEL) *
        (tData[pixelIdx] >= tUThresh || iData[pixelIdx] >= iUThresh);
    mData[pixelIdx] |=
        (FLAG_INPUT_ISBAD | FLAG_LOW_PIXEL) *
        (tData[pixelIdx] <= tLThresh || iData[pixelIdx] <= iLThresh);
  }
...
#define FSTRNCMP(a, b, n) \
  ...
  /* copy remaining keywords, excluding NAXIS?, EXTEND, and reference COMMENT
   * keywords */
  for (i = 4 + naxis; i <= nkeys; i++) {
    if (fits_read_record(iPtr, i, card, status)) break;
    ...
    if (FSTRNCMP(card, "EXTEND  ", 8) &&
      ...
      if (fits_write_record(oPtr, card, status)) return *status;
    }
  }
  return *status;
}
 ...
 */
void hp_fits_correct_data(float* data, int npix, float bZero, float bScale,
                          int makeShort) {
 ...
 * @param makeShort  If non-zero, apply short-integer range clamping.
 */
void hp_fits_correct_data_int(int* data, int npix, float bZero, float bScale,
                              int makeShort) {
  float maxval = 1e30, minval = -1e30;
  int i;
  int *dptr, *mptr;
...
int hp_fits_write_subset(fitsfile* fptr, long group, long naxis, long* naxes,
                         ...
                         int lpixelY, int xArrayLo, int yArrayLo) {
  int pixX, pixY, y, x;
  float* dptr;
  ...
  for (y = 0; y < pixY; y++) {
    ...
    /* this works! */
    fits_write_subset_flt(fptr, group, naxis, naxes, fpixel, lpixel, dptr,
                          status);
 ...
 * @return cfitsio status code.
 */
int hp_fits_write_subset_int(fitsfile* fptr, long group, long naxis,
                             long* naxes, int* data, int* status, int makeShort,
                             float bZero, float bScale, int fpixelX,
                             int fpixelY, int lpixelX, int lpixelY,
  ...
  for (y = 0; y < pixY; y++) {
    ...
    /* this works! */
    fits_write_subset_int(fptr, group, naxis, naxes, fpixel, lpixel, dptr,
                          status);
