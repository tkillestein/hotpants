...
int main(int argc, char* argv[]) {
  ...
  double meansigSubstamps, scatterSubstamps; /* mean sigma and scatter of
                                                substamps, for fits header */
  double meansigSubstampsF,
      scatterSubstampsF; /* mean sigma and scatter of substamps, using final
                            noise */
  ...
  /*
    NOTE : The .fits images are 1-indexed, while these arrays contained within
    are 0 indexed.  Thus anything you find below at x,y you will find displayed
    at position x+1,y+1.
  */
  ...
  if (fits_open_file(&tPtr, template, 0, &status)) printError(status);
  ...
  if (tNoiseIm) {
    if (fits_open_file(&ePtr, tNoiseIm, 0, &status)) printError(status);
    if (fits_get_img_param(ePtr, MAXDIM, &oBitpix, &oNaxis, oNaxes, &status))
      printError(status);

    if ((oNaxes[0] != tNaxes[0]) || (oNaxes[1] != tNaxes[1])) {
      LOG_WARNING(
          "WARNING : Input template noise array not same size as template, "
          "ignoring...");
      tNoiseIm = NULL;
    }

    if (fits_close_file(ePtr, &status)) printError(status);
  }

  /* open up, get comparison image bitpix, # dimensions, image size */
  if (fits_open_file(&iPtr, image, 0, &status)) printError(status);
  ...
  /* use the ePtr fitsfile temporarily */
  if (iNoiseIm) {
    if (fits_open_file(&ePtr, iNoiseIm, 0, &status)) printError(status);
    if (fits_get_img_param(ePtr, MAXDIM, &oBitpix, &oNaxis, oNaxes, &status))
      printError(status);

    if ((oNaxes[0] != iNaxes[0]) || (oNaxes[1] != iNaxes[1])) {
      LOG_WARNING(
          "WARNING : Input image noise array not same size as image, "
          "ignoring...");
      iNoiseIm = NULL;
    }
    if (fits_close_file(ePtr, &status)) printError(status);
  ...
  if (tNoiseIm) {
    ...
    if ((oNaxes[0] != iNaxes[0]) || (oNaxes[1] != iNaxes[1])) {
      LOG_WARNING(
          "WARNING : Input template noise array not same size as image, "
          "ignoring...");
      tNoiseIm = NULL;
    }
    if (fits_close_file(ePtr, &status)) printError(status);
  ...
  if (iMaskIm) {
    if (fits_open_file(&ePtr, iMaskIm, 0, &status)) printError(status);
    if (fits_get_img_param(ePtr, MAXDIM, &oBitpix, &oNaxis, oNaxes, &status))
      printError(status);

    if ((oNaxes[0] != iNaxes[0]) || (oNaxes[1] != iNaxes[1])) {
      LOG_WARNING(
          "WARNING : Input image mask array not same size as image, "
          "ignoring...");
      iMaskIm = NULL;
    }
    if (fits_close_file(ePtr, &status)) printError(status);
  ...
  if (tMaskIm) {
    if (fits_open_file(&ePtr, tMaskIm, 0, &status)) printError(status);
    if (fits_get_img_param(ePtr, MAXDIM, &oBitpix, &oNaxis, oNaxes, &status))
      printError(status);

    if ((oNaxes[0] != iNaxes[0]) || (oNaxes[1] != iNaxes[1])) {
      LOG_WARNING(
          "Input template mask array not same size as image, ignoring...");
      tMaskIm = NULL;
    }
    if (fits_close_file(ePtr, &status)) printError(status);
  }
  ...
  LOG_PROGRESS("Doing : %s - %s = %s", image, template, outim);
  LOG_PROGRESS("Good templ data : %.1f -> %.1f", tLThresh, tUThresh);
  LOG_PROGRESS("Good image data : %.1f -> %.1f", iLThresh, iUThresh);
  ...
  if (!noClobber) {
    snprintf(scrStr, sizeof(scrStr), "!%s", outim);
  } else {
    snprintf(scrStr, sizeof(scrStr), "%s", outim);
  }
  ...
  if (hp_fits_copy_header(iPtr, oPtr, &status)) printError(status);
  ...
  fits_write_key_str(oPtr, "SOFTAUTH",
                     "Andrew Becker <becker@astro.washington.edu>",
                     "Maintainer", &status);
  fits_write_key_str(oPtr, "SOFTINST", "University of Washington", "Institute",
                     &status);
  fits_write_key_str(oPtr, "COMMENT", "", "", &status);
  ...
  if (convImage) {
    if (!noClobber) {
      snprintf(scrStr, sizeof(scrStr), "!%s", convImage);
    } else {
      snprintf(scrStr, sizeof(scrStr), "%s", convImage);
    }
  ...
  if (sigmaImage) {
    if (!noClobber) {
      snprintf(scrStr, sizeof(scrStr), "!%s", sigmaImage);
    } else {
      snprintf(scrStr, sizeof(scrStr), "%s", sigmaImage);
    }
    if (fits_create_file(&ePtr, scrStr, &status) ||
  ...
  if (noiseImage) {
    if (!noClobber) {
      snprintf(scrStr, sizeof(scrStr), "!%s", noiseImage);
    } else {
      snprintf(scrStr, sizeof(scrStr), "%s", noiseImage);
    }

    if (fits_create_file(&ePtr, scrStr, &status)) printError(status);
    ...
    if (hp_fits_copy_header(iPtr, ePtr, &status) ||
        fits_update_key(ePtr, TSTRING, "OBJECT", "HOTPanTS Noise Image", "",
                        &status) ||
        fits_update_key_flt(ePtr, "GAIN", 1., -1, "No gain in noise image",
                            &status) ||
  ...
  if (outMask) {
    if (!noClobber) {
      snprintf(scrStr, sizeof(scrStr), "!%s", outMask);
    } else {
      snprintf(scrStr, sizeof(scrStr), "%s", outMask);
    }
    if (fits_create_file(&ePtr, scrStr, &status) ||
        ...
        hp_fits_copy_header(iPtr, ePtr, &status) ||
        fits_update_key(ePtr, TSTRING, "OBJECT", "Hotpants output Mask Image",
                        "", &status) ||
  ...
  if (kernelImIn) {
    ...
    LOG_DEBUG("Received kernel info");
  }
  ...
  if (useFullSS) {
    ...
    LOG_PROGRESS("Using maximial number of stamps : %d x %d", nStampX, nStampY);
  } else {
    ...
    if (fwStamp < fwKSStamp) {
      ...
      LOG_WARNING("Too many stamps requested");
      LOG_WARNING("Using nsx = %d, nsy = %d", nStampX, nStampY);
    }
  }
  ...
  LOG_PROGRESS("Mallocing massive amounts of memory...");
...
#pragma omp parallel
  ...
  xMax = -1 + (gdXmax ? imin(imin(tNaxes[0], iNaxes[0]), gdXmax)
                      : imin(tNaxes[0], iNaxes[0]));
  yMax = -1 + (gdYmax ? imin(imin(tNaxes[1], iNaxes[1]), gdYmax)
  ...
  if (regFile) {
    ...
    for (nR = 0; nR < numRegKeyWord; nR++) {
      ...
      if (fits_read_key_str(tPtr, hKeyword, hInfo, scrStr, &status)) {
        ...
        sscanf(hInfo, "[%d:%d,%d:%d]", &rXMins[nR], &rXMaxs[nR], &rYMins[nR],
               &rYMaxs[nR]);
      ...
      /* LOG_PROGRESS("%d %d %d %d\n", rXMins[nR], rYMins[nR], rXMaxs[nR],
       * rYMaxs[nR]); */
    }
  } else {
  ...
  if ((doKerInfo) || (kernelImOut)) {
    ...
    if (doKerInfo) {
       ...
       */
      if (fits_insert_btbl(oPtr, (nCompTotal + 1), nR, ttype, tform, tunit,
                           "Convolution Kernel Information", 0L, &status) ||
          fits_update_key(oPtr, TSTRING, "OBJECT",
    ...
    if (kernelImOut) {
      if (!noClobber) {
        snprintf(scrStr, sizeof(scrStr), "!%s", kernelImOut);
      } else {
        snprintf(scrStr, sizeof(scrStr), "%s", kernelImOut);
      }
      if (fits_create_file(&ePtr, scrStr, &status) ||
          fits_insert_btbl(ePtr, (nCompTotal + 1), nR, ttype, tform, tunit,
                           "Convolution Kernel Information", 0L, &status) ||
          fits_update_key(ePtr, TSTRING, "OBJECT",
                          "Convolution Kernel Information", "", &status) ||
...
#ifdef _OPENMP
#pragma omp parallel for schedule(dynamic) copyin(kerFitThresh) private(       \
        j, k, l, m, rXMin, rYMin, rXMax, rYMax, rXBMin, rYBMin, rXBMax,        \
  ...
  for (i = 0; i < nR; i++) {
    ...
    if (kernelImIn) {
      ...
      readKernel(kernelImIn, i, &tKerSol, &iKerSol, &rXMin, &rXMax, &rYMin,
                 &rYMax, &meansigSubstamps, &scatterSubstamps,
                 &meansigSubstampsF, &scatterSubstampsF, &diffrat,
                 &NskippedSubstamps);
    } else {
      rXMin = rXMins[i];
      rXMax = rXMaxs[i];
    ...
    LOG_PROGRESS("Region %d pixels            : %ld:%ld,%ld:%ld", i, pixMin[0],
                 pixMax[0], pixMin[1], pixMax[1]);
    LOG_PROGRESS("Vector Indices (buffered) : %d:%d,%d:%d", rXBMin, rXBMax,
                 rYBMin, rYBMax);
...
#pragma omp critical(cfitsio)
    {
      if (fits_read_subset_flt(tPtr, 0, tNaxis, tNaxes, pixMin, pixMax, inc, 0,
                               tRData, &anynul, &status) ||
          fits_read_subset_flt(iPtr, 0, iNaxis, iNaxes, pixMin, pixMax, inc, 0,
                               iRData, &anynul, &status)) {
        printError(status);
      }
    }
    ...
    if (iNoiseIm) {
      if (fits_open_file(&ePtr, iNoiseIm, 0, &status)) printError(status);
      if (fits_read_subset_flt(ePtr, 0, iNaxis, iNaxes, pixMin, pixMax, inc, 0,
                               oRData, &anynul, &status) ||
          fits_close_file(ePtr, &status))
        printError(status);

      for (l = rPixX * rPixY; l--;) oRData[l] *= oRData[l];
    } else {
    ...
    if (tNoiseIm) {
      if (fits_open_file(&ePtr, tNoiseIm, 0, &status)) printError(status);
      if (fits_read_subset_flt(ePtr, 0, tNaxis, tNaxes, pixMin, pixMax, inc, 0,
                               eRData, &anynul, &status) ||
          fits_close_file(ePtr, &status))
        printError(status);

      for (l = rPixX * rPixY; l--;) eRData[l] *= eRData[l];
    } else {
    ...
    for (l = rPixX * rPixY; l--;) oRData[l] += eRData[l];
    ...
    */
    if (iMaskIm) {
      if (fits_open_file(&ePtr, iMaskIm, 0, &status)) printError(status);
      if (fits_read_subset_int(ePtr, 0, tNaxis, tNaxes, pixMin, pixMax, inc, 0,
                               misRData, &anynul, &status) ||
          fits_close_file(ePtr, &status))
        printError(status);
    ...
    /* make sure program can't find a valid stamp center around the edge of the
     * image */
    ...
    if (!(kernelImIn)) {
      ...
      while ((status <= 2) && (flag)) {
        ...
        for (l = 0; l < nStampY; l++) {
          for (k = 0; k < nStampX; k++) {
            LOG_PROGRESS("Build stamp: t %4d i %4d (grid coord %2d %2d)", ntS,
                         niS, k, l);
            ...
            if (sstampFile) {
              LOG_DEBUG("Adding centers manually");
              for (m = 0; m < Ncmp; m++) {
                if ((xcmp[m] > sXMin + hwKernel + 1) &&
                    (xcmp[m] < sXMax - hwKernel - 1) &&
                    (ycmp[m] > sYMin + hwKernel + 1) &&
                    (ycmp[m] < sYMax - hwKernel - 1)) {
                  buildStamps(sXMin, sXMax, sYMin, sYMax, &niS, &ntS, 0, rXBMin,
                              rYBMin, ciStamps, ctStamps, iRData, tRData,
                              xcmp[m] - rXBMin, ycmp[m] - rYBMin);
                }
              }
              if (findSSC) {
                LOG_DEBUG("Automatically finding additional centers");
                /* and then choose enough stamps manually to fill the array */
                buildStamps(sXMin, sXMax, sYMin, sYMax, &niS, &ntS, 1, rXBMin,
                            rYBMin, ciStamps, ctStamps, iRData, tRData, 0, 0);
              }
            } else {
              if (useFullSS) {
            ...
            if (!(strncmp(forceConvolve, "i", 1) == 0)) {
              LOG_DEBUG("Template: %d substamps", ctStamps[ntS].nss);
              if (ctStamps[ntS].nss > 0) ntS += 1;
            }

            if (!(strncmp(forceConvolve, "t", 1) == 0)) {
              LOG_DEBUG("Image: %d substamps", ciStamps[niS].nss);
        ...
        if (strncmp(forceConvolve, "i", 1) == 0) {
          LOG_PROGRESS("%d stamps built (%.2f%s)", niS, iSFrac, "%");
          if (iSFrac < minFracGoodStamps) flag = 1;
        } else if (strncmp(forceConvolve, "t", 1) == 0) {
          LOG_PROGRESS("%d stamps built (%.2f%s)", ntS, tSFrac, "%");
          if (tSFrac < minFracGoodStamps) flag = 1;
        } else if ((iSFrac < minFracGoodStamps) ||
                   (tSFrac < minFracGoodStamps)) {
          LOG_PROGRESS("%d and %d stamps built (%.2f%s, %.2f%s)", ntS, niS,
                       tSFrac, "%", iSFrac, "%");
          flag = 1;
        } else {
          LOG_PROGRESS("%d and %d stamps built (%.2f%s, %.2f%s)", ntS, niS,
        ...
        */
        ...
        if ((flag) && (status <= 1) && (scaleFitThresh < 1.)) {
          ...
          for (l = rPixX * rPixY; l--;) mRData[l] &= ~0xa00;
        }
        /* loop */
        status += 1;
      }
      ...
      */
      ...
      if (!(strncmp(forceConvolve, "i", 1) == 0)) {
        ...
        if ((strncmp(forceConvolve, "b", 1) == 0)) {
          LOG_PROGRESS("Trying to convolve the TEMPLATE to fit IMAGE");
          tMerit = check_stamps(ctStamps, ntS, iRData, oRData);
          LOG_PROGRESS("Result: merit = %.3f", tMerit);
        } else
          tMerit = iMerit = 0;
      }

      LOG_PROGRESS("Filling image sub-stamps");
      if (!(strncmp(forceConvolve, "t", 1) == 0)) {
        for (k = 0; k < niS; k++) {
          ciStamps[k].sscnt = 0;
        ...
        if ((strncmp(forceConvolve, "b", 1) == 0)) {
          LOG_PROGRESS("Trying to convolve the IMAGE to fit TEMPLATE");
          iMerit = check_stamps(ciStamps, niS, tRData, oRData);
          LOG_PROGRESS("Result: merit = %.3f", iMerit);
        } else
          iMerit = tMerit = 0;
      }
    ...
    if (convTmpl) {
      ...
      for (l = rPixX * rPixY; l--;) {
        mtsRData[l] |=
            (FLAG_INPUT_ISBAD | FLAG_BAD_PIXVAL) * (tRData[l] == fillVal);
        mtsRData[l] |=
            (FLAG_INPUT_ISBAD | FLAG_SAT_PIXEL) * (tRData[l] >= tUThresh);
        mtsRData[l] |=
            (FLAG_INPUT_ISBAD | FLAG_LOW_PIXEL) * (tRData[l] <= tLThresh);
      }
      ...
      if (tNoiseIm) {
        if (fits_open_file(&ePtr, tNoiseIm, 0, &status)) printError(status);
        if (fits_read_subset_flt(ePtr, 0, tNaxis, tNaxes, pixMin, pixMax, inc,
                                 0, eRData, &anynul, &status) ||
            fits_close_file(ePtr, &status))
          printError(status);
      ...
      if (iNoiseIm) {
        if (fits_open_file(&ePtr, iNoiseIm, 0, &status)) printError(status);
        if (fits_read_subset_flt(ePtr, 0, iNaxis, iNaxes, pixMin, pixMax, inc,
                                 0, tRData, &anynul, &status) ||
            fits_close_file(ePtr, &status))
          printError(status);

        for (l = rPixX * rPixY; l--;) tRData[l] *= tRData[l];
      } else {
        tRData = makeNoiseImage4(iRData, 1. / iGain, iRdnoise / iGain);
      }

      /* add noise portions to create final noise image */
      for (l = rPixX * rPixY; l--;) tRData[l] = sqrt(tRData[l] + eRData[l]);
      free(eRData);
      eRData = NULL;
      ...
      for (l = rPixX * rPixY; l--;) {
        mRData[l] |= (FLAG_OUTPUT_ISBAD | FLAG_INPUT_ISBAD | FLAG_BAD_PIXVAL) *
                     (iRData[l] == fillVal);
        mRData[l] |= (FLAG_OUTPUT_ISBAD | FLAG_INPUT_ISBAD | FLAG_SAT_PIXEL) *
                     (iRData[l] >= iUThresh);
        mRData[l] |= (FLAG_OUTPUT_ISBAD | FLAG_INPUT_ISBAD | FLAG_LOW_PIXEL) *
                     (iRData[l] <= iLThresh);
        mRData[l] |= misRData[l];
      ...
      oRData   : Convolved input template image AND difference image AND sigma
      image tRData   : Noise image iRData   : Input image
      ...
      */
      ...
      /* and oRData is the input noise^2 */
      if (!(kernelImIn))
        fitKernel(ciStamps, tRData, iRData, oRData, iKerSol, &meansigSubstamps,
                  &scatterSubstamps, &NskippedSubstamps);
      ...
      for (l = rPixX * rPixY; l--;) {
        misRData[l] |=
            (FLAG_INPUT_ISBAD | FLAG_BAD_PIXVAL) * (iRData[l] == fillVal);
        misRData[l] |=
            (FLAG_INPUT_ISBAD | FLAG_SAT_PIXEL) * (iRData[l] >= iUThresh);
        misRData[l] |=
            (FLAG_INPUT_ISBAD | FLAG_LOW_PIXEL) * (iRData[l] <= iLThresh);
      }
       ...
       * global mRData output mask!  bitwise... */
      LOG_PROGRESS("Convolving...");
      spatial_convolve(iRData, &eRData, rPixX, rPixY, iKerSol, oRData,
                       misRData);
      ...
      if (tNoiseIm) {
        if (fits_open_file(&ePtr, tNoiseIm, 0, &status)) printError(status);
        if (fits_read_subset_flt(ePtr, 0, tNaxis, tNaxes, pixMin, pixMax, inc,
                                 0, iRData, &anynul, &status) ||
            fits_close_file(ePtr, &status))
          printError(status);

        for (l = rPixX * rPixY; l--;) iRData[l] *= iRData[l];
      } else {
        iRData = makeNoiseImage4(tRData, 1. / tGain, tRdnoise / tGain);
      }

      /* add noise portions to create final noise image */
      for (l = rPixX * rPixY; l--;) iRData[l] = sqrt(iRData[l] + eRData[l]);
      free(eRData);
      eRData = NULL;
    ...
    for (l = 0; l < rPixY; l++) {
      for (k = 0; k < hwKernel; k++) mRData[k + rPixX * l] |= FLAG_OUTPUT_ISBAD;
    ...
    LOG_PROGRESS("Creating and writing output images...");
...
#pragma omp critical(cfitsio)
        {
          if (fits_movabs_hdu(oPtr, 2, NULL, &status)) printError(status);
          if (outShort)
      ...
      if (convImage) {
        if (fits_open_file(&ePtr, convImage, 1, &status)) printError(status);
        if (outShort)
          if (fits_set_bscale(ePtr, outBscale, outBzero, &status))
            printError(status);

        if (hp_fits_write_subset(ePtr, 0, 2, oNaxes, oRData, &status, outShort,
                                 outBzero, outBscale, fpixelOutX, fpixelOutY,
        ...
        if (strncmp(figMerit, "v", 1) == 0) {
          ...
          for (l = 0; l < nS; l++) {
            ...
            if (ctStamps[l].sscnt < ctStamps[l].nss) {
              ...
              temp2[k++] = sum;
              /*LOG_PROGRESS("SSSIG %d : %f\n", k, sum);*/
            }
          }
          /* save the mean and scatter so that it can be saved in the fits
           * header */
          sigma_clip(temp2, k, &meansigSubstampsF, &scatterSubstampsF, 10);
          LOG_PROGRESS("FINAL mean sig: %6.3f stdev: %6.3f", meansigSubstampsF,
                       scatterSubstampsF);
          free(temp2);
        }
      }
    ...
    LOG_PROGRESS("Getting diffim stats for GOOD pixels:");
    getStampStats3(oRData, 0, 0, rPixX, rPixY, &sum, &mean, &median, &mode, &sd,
                   &fwhm, &lfwhm, 0x0, 0xffff, 5);
    LOG_PROGRESS("Mean   : %.2f", mean);
    LOG_PROGRESS("Median : %.2f", median);
    LOG_PROGRESS("Mode   : %.2f", mode);
    LOG_PROGRESS("Stdev  : %.2f", sd);
    LOG_DEBUG("FWHM   : %.2f", fwhm);
    LOG_DEBUG("lFWHM  : %.2f", lfwhm);

    LOG_PROGRESS("Getting noiseim stats for GOOD pixels:");
    if (convTmpl) {
      getStampStats3(tRData, 0, 0, rPixX, rPixY, &nsum, &nmean, &nmedian,
                     &nmode, &nsd, &nfwhm, &nlfwhm, 0x0, 0xffff, 5);
      getNoiseStats3(oRData, tRData, &x2norm, &nx2norm, 0x0, 0xffff);
    }

    else {
      getStampStats3(iRData, 0, 0, rPixX, rPixY, &nsum, &nmean, &nmedian,
                     &nmode, &nsd, &nfwhm, &nlfwhm, 0x0, 0xffff, 5);
      getNoiseStats3(oRData, iRData, &x2norm, &nx2norm, 0x0, 0xffff);
    }

    LOG_PROGRESS("Mean   : %.2f", nmean);
    LOG_PROGRESS("Median : %.2f", nmedian);
    LOG_PROGRESS("Mode   : %.2f", nmode);
    LOG_PROGRESS("Stdev  : %.2f", nsd);
    LOG_DEBUG("FWHM   : %.2f", nfwhm);
    LOG_DEBUG("lFWHM  : %.2f", nlfwhm);

    /* find ratio for GOOD pixels only */
    if (!(kernelImIn))
      LOG_PROGRESS("Empirical / Expected Noise for `good` pixels = %.2f",
                   sd / nmean);
    LOG_PROGRESS("X2NORM = %.2f", x2norm);
    ...
    LOG_PROGRESS("Getting diffim stats for `ok` pixels:");
    getStampStats3(oRData, 0, 0, rPixX, rPixY, &summ, &meanm, &medianm, &modem,
                   &sdm, &fwhmm, &lfwhmm, 0xff, FLAG_OUTPUT_ISBAD, 5);
    LOG_PROGRESS("Mean:   %.2f", meanm);
    LOG_PROGRESS("Median: %.2f", medianm);
    LOG_PROGRESS("Mode:   %.2f", modem);
    LOG_PROGRESS("Stdev:  %.2f", sdm);
    LOG_DEBUG("FWHM:      %.2f", fwhmm);
    LOG_DEBUG("lFWHM:     %.2f", lfwhmm);

    LOG_PROGRESS("Getting noiseim stats for OK pixels:");
    if (convTmpl)
      getStampStats3(tRData, 0, 0, rPixX, rPixY, &nsumm, &nmeanm, &nmedianm,
                     &nmodem, &nsdm, &nfwhmm, &nlfwhmm, 0xff, FLAG_OUTPUT_ISBAD,
                     5);
    else
      getStampStats3(iRData, 0, 0, rPixX, rPixY, &nsumm, &nmeanm, &nmedianm,
                     &nmodem, &nsdm, &nfwhmm, &nlfwhmm, 0xff, FLAG_OUTPUT_ISBAD,
                     5);

    LOG_PROGRESS("Mean:   %.2f", nmeanm);
    LOG_PROGRESS("Median: %.2f", nmedianm);
    LOG_PROGRESS("Mode:   %.2f", nmodem);
    LOG_PROGRESS("Stdev:  %.2f", nsdm);
    LOG_DEBUG("FWHM:      %.2f", nfwhmm);
    LOG_DEBUG("lFWHM:     %.2f", nlfwhmm);

    /* find ratio for GOOD pixels only */
    if (!(kernelImIn))
      LOG_PROGRESS("Empirical / Expected Noise for OK pixels = %.2f",
                   sdm / nmeanm);
    ...
    if (rescaleOK) {
      ...
      if (diffrat > 1) {
        LOG_PROGRESS("Scale OK pixel noise by = %.2f", diffrat);
        if (convTmpl)
          for (l = rPixX * rPixY; l--;) {
        ...
        LOG_PROGRESS("Leave OK pixel noise as-is");
    }
...
#pragma omp critical(cfitsio)
    {
      ...
      if (fits_movabs_hdu(oPtr, 1, NULL, &status) ||
          hp_fits_write_subset(oPtr, 0, 2, oNaxes, oRData, &status, outShort,
                               outBzero, outBscale, fpixelOutX, fpixelOutY,
                               lpixelOutX, lpixelOutY, xBufLo, yBufLo))
        printError(status);
      ...
      if (inclSigmaImage || sigmaImage) {
        ...
        if (sigmaImage) {
          if (fits_open_file(&ePtr, sigmaImage, 1, &status)) printError(status);
      ...
      if (inclNoiseImage) {
        ...
        if (convTmpl) {
          if (hp_fits_write_subset(oPtr, 0, 2, oNaxes, tRData, &status,
                                   outNShort, outNiBzero, outNiBscale,
                                   fpixelOutX, fpixelOutY, lpixelOutX,
                                   lpixelOutY, xBufLo, yBufLo))
            printError(status);
        } else {
      ...
      if (noiseImage) {
        if (fits_open_file(&ePtr, noiseImage, 1, &status)) printError(status);
      ...
      if (outMask) {
        if (fits_open_file(&ePtr, outMask, 1, &status)) printError(status);
        if (hp_fits_write_subset_int(ePtr, 0, 2, oNaxes, mRData, &status, 1,
                                     32768, 1, fpixelOutX, fpixelOutY,
                                     lpixelOutX, lpixelOutY, xBufLo, yBufLo) ||
            fits_close_file(ePtr, &status))
          printError(status);
      }
      ...
      if (kernelImOut)
        if (fits_open_file(&ePtr, kernelImOut, 1, &status)) printError(status);

      /* add comment to TITLE saying which image was actually convolved */
      snprintf(hKeyword, sizeof(hKeyword), "REGION%02d", i);
      snprintf(hInfo, sizeof(hInfo), "[%d:%d,%d:%d]", rXMin + 1, rXMax + 1,
               rYMin + 1, rYMax + 1);
      fits_write_key_str(oPtr, hKeyword, hInfo, "", &status);
      if (kernelImOut) fits_write_key_str(ePtr, hKeyword, hInfo, "", &status);
      ...
      fits_write_key_str(oPtr, hKeyword, hInfo, "Stdev in Figure of Merit",
                         &status);
      if (kernelImOut)
        fits_write_key_str(ePtr, hKeyword, hInfo, "Stdev in Figure of Merit",
                           &status);
      ...
      fits_write_key_str(oPtr, hKeyword, hInfo, "1/N * SUM (diff/noise)^2",
                         &status);
      if (kernelImOut)
        fits_write_key_str(ePtr, hKeyword, hInfo, "1/N * SUM (diff/noise)^2",
                           &status);
      ...
      fits_write_key_str(oPtr, hKeyword, hInfo, "Number of pixels in X2NRM",
                         &status);
      if (kernelImOut)
        fits_write_key_str(ePtr, hKeyword, hInfo, "Number of pixels in X2NRM",
                           &status);
      ...
      fits_write_key_str(oPtr, hKeyword, hInfo, "Mean of diff image; OK pixels",
                         &status);
      if (kernelImOut)
        fits_write_key_str(ePtr, hKeyword, hInfo,
      ...
      if (kernelImOut) fits_close_file(ePtr, &status);

      if (doKerInfo) {
        if (convTmpl) {
          if (fits_movabs_hdu(oPtr, kInfoNum, NULL, &status) ||
              fits_write_col_dbl(oPtr, i + 1, 1, 1, (nCompTotal + 1), tKerSol,
                                 &status))
            printError(status);
        } else {
          if (fits_movabs_hdu(oPtr, kInfoNum, NULL, &status) ||
              fits_write_col_dbl(oPtr, i + 1, 1, 1, (nCompTotal + 1), iKerSol,
                                 &status))
            printError(status);
        }
      }
      if (kernelImOut) {
        if (convTmpl) {
          if (fits_open_file(&ePtr, kernelImOut, 1, &status) ||
              ...
              fits_write_col_dbl(ePtr, i + 1, 1, 1, (nCompTotal + 1), tKerSol,
                                 &status) ||
              fits_close_file(ePtr, &status))
            printError(status);
        } else {
          if (fits_open_file(&ePtr, kernelImOut, 1, &status) ||
              fits_movabs_hdu(ePtr, kInfoNum, NULL, &status) ||
              fits_write_col_dbl(ePtr, i + 1, 1, 1, (nCompTotal + 1), iKerSol,
                                 &status) ||
              fits_close_file(ePtr, &status))
            printError(status);
        }
...
#pragma omp parallel
  ...
  if (doKerInfo) {
    ...
    for (k = 0; k < ngauss; k++) {
      ...
      fits_write_key_flt(oPtr, hKeyword, 1. / sqrt(2. * sigma_gauss[k]), 8,
                         "Gaussian Sigma", &status);
    }

    fits_write_key_lng(oPtr, "FWKERN", fwKernel, "Kernel Width in Pixels",
                       &status);
    fits_write_key_lng(oPtr, "CKORDER", kerOrder, "Spatial Convolution Order",
                       &status);
    fits_write_key_lng(oPtr, "BGORDER", bgOrder, "Background Order", &status);
  } else {
    if (!(kernelImOut)) fits_write_key_log(oPtr, "KERINFO", 0, "", &status);
  }
  ...
  if (kernelImOut) {
    ...
    for (k = 0; k < ngauss; k++) {
      ...
      snprintf(hKeyword, sizeof(hKeyword), "SGAUSS%d", k + 1);
      fits_write_key_flt(ePtr, hKeyword, 1. / sqrt(2. * sigma_gauss[k]), 8,
                         "Gaussian Sigma", &status);
      fits_write_key_flt(oPtr, hKeyword, 1. / sqrt(2. * sigma_gauss[k]), 8,
                         "Gaussian Sigma", &status);
    }
    ...
    fits_write_key_lng(ePtr, "FWKERN", fwKernel, "Kernel Width in Pixels",
                       &status);
    fits_write_key_lng(ePtr, "CKORDER", kerOrder, "Spatial Convolution Order",
                       &status);
