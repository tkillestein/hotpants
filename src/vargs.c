...
#define HELP_APPEND(fmt, ...) \
  ...
  HELP_APPEND(
      "   [-tu tuthresh]    : upper valid data count, template (%.0f)\n",
      D_UTHRESH);
  HELP_APPEND(
      "   [-tuk tucthresh]  : upper valid data count for kernel, template "
      "(tuthresh)\n");
  HELP_APPEND(
      "   [-tl tlthresh]    : lower valid data count, template (%.0f)\n",
      D_LTHRESH);
  ...
  HELP_APPEND("   [-iu iuthresh]    : upper valid data count, image (%.0f)\n",
              D_UTHRESH);
  HELP_APPEND(
      "   [-iuk iucthresh]  : upper valid data count for kernel, image "
      "(iuthresh)\n");
  ...
  HELP_APPEND("   [-r rkernel]      : convolution kernel half width (%d)\n",
              hwKernel);
  HELP_APPEND(
      "   [-kcs step]       : size of step for spatial convolution (2*rkernel "
      "+ 1)\n");
  HELP_APPEND(
      "   [-ft fitthresh]   : RMS threshold for good centroid in kernel fit "
      "(%.1f)\n",
      kerFitThresh);
  HELP_APPEND(
      "   [-sft scale]      : scale fitthresh by this fraction if... (%.1f)\n",
      scaleFitThresh);
  HELP_APPEND(
      "   [-nft fraction]   : this fraction of stamps are not filled (%.1f)\n",
      minFracGoodStamps);

  HELP_APPEND(
      "   [-mins spread]    : Fraction of kernel half width to spread input "
      "mask (%.1f)\n",
      kfSpreadMask1);
  HELP_APPEND(
      "   [-mous spread]    : Ditto output mask, negative = no diffim masking "
      "(%.1f)\n",
      kfSpreadMask2);
  HELP_APPEND("   [-omi  fitsfile]  : Output bad pixel mask (undef)\n");

  HELP_APPEND(
      "   [-nrx xregion]    : number of image regions in x dimension (%d)\n",
      nRegX);
  HELP_APPEND(
      "   [-nry yregion]    : number of image regions in y dimension (%d)\n",
      nRegY);
  HELP_APPEND("   -- OR --\n");
  HELP_APPEND(
      "   [-rf regionfile]  : ascii file with image regions "
      "'xmin:xmax,ymin:ymax'\n");
  HELP_APPEND("   -- OR --\n");
  HELP_APPEND(
      "   [-rkw keyword num]: header 'keyword[0->(num-1)]' indicates valid "
      "regions\n\n");

  HELP_APPEND(
      "   [-nsx xstamp]     : number of each region's stamps in x dimension "
      "(%d)\n",
      nStampX);
  HELP_APPEND(
      "   [-nsy ystamp]     : number of each region's stamps in y dimension "
      "(%d)\n",
      nStampY);
  HELP_APPEND("   -- OR --\n");
  HELP_APPEND(
  ...
  HELP_APPEND(
      "   [-afssc find]     : autofind stamp centers so #=-nss when -ssf,-cmp "
      "(%d)\n",
      findSSC);
  HELP_APPEND(
      "   [-nss substamps]  : number of centroids to use for each stamp (%d)\n",
      nKSStamps);
  HELP_APPEND(
      "   [-rss radius]     : half width substamp to extract around each "
      "centroid (%d)\n\n",
      hwKSStamp);

  HELP_APPEND(
      "   [-savexy file]    : save positions of stamps for convolution kernel "
      "(undef)\n");

  HELP_APPEND(
      "   [-c  toconvolve]  : force convolution on (t)emplate or (i)mage "
      "(undef)\n");
  HELP_APPEND(
      "   [-n  normalize]   : normalize to (t)emplate, (i)mage, or "
      "(u)nconvolved (%s)\n",
      photNormalize);
  HELP_APPEND(
      "   [-fom figmerit]   : (v)ariance, (s)igma or (h)istogram convolution "
      "merit (%s)\n",
      figMerit);
  HELP_APPEND(
      "   [-ko kernelorder] : spatial order of kernel variation within region "
      "(%d)\n",
      kerOrder);
  HELP_APPEND(
  ...
  HELP_APPEND(
      ...
      statSig);
  HELP_APPEND(
      "   [-ks badkernelsig]: high sigma rejection for bad stamps in kernel "
      "fit (%.1f)\n",
      kerSigReject);
  HELP_APPEND(
      "   [-kfm kerfracmask]: fraction of abs(kernel) sum for ok pixel "
      "(%.3f)\n",
      kerFracMask);
  ...
  HELP_APPEND(
      "   [-nim]            : add noise image as layer to sub image (%d)\n",
      inclNoiseImage);
  HELP_APPEND(
      "   [-ndm]            : add noise-scaled sub image as layer to sub image "
      "(%d)\n\n",
      inclSigmaImage);
  HELP_APPEND("   [-oci fitsfile]   : output convolved image (undef)\n");
  HELP_APPEND(
      "   [-cim]            : add convolved image as layer to sub image "
      "(%d)\n\n",
      inclConvImage);
  ...
  HELP_APPEND("   [-nc]             : do not clobber output image (%d)\n",
              noClobber);
  HELP_APPEND(
      "   [-hki]            : print extensive kernel info to output image "
      "header (%d)\n\n",
      doKerInfo);
  HELP_APPEND(
      "   [-oki fitsfile]   : new fitsfile with kernel info (under)\n\n");
  HELP_APPEND(
      "   [-sht]            : output images 16 bitpix int, vs -32 bitpix float "
      "(%d)\n",
      outShort);
  HELP_APPEND(
      "   [-obs bscale]     : if -sht, output image BSCALE, overrides -inim "
      "(%.1f)\n",
      D_OUTBSCALE);
  HELP_APPEND(
      "   [-obz bzero]      : if -sht, output image BZERO , overrides -inim "
      "(%.1f)\n",
      D_OUTBZERO);
  HELP_APPEND(
      "   [-nsht]           : output noise image 16 bitpix int, vs -32 bitpix "
      "float (%d)\n",
      outNShort);
  HELP_APPEND(
      "   [-nbs bscale]     : noise image only BSCALE, overrides -obs (%.1f)\n",
      D_OUTBSCALE);
  HELP_APPEND(
      "   [-nbz bzero]      : noise image only BZERO,  overrides -obz "
      "(%.1f)\n\n",
      D_OUTBZERO);

  HELP_APPEND("   [-ng  ngauss degree0 sigma0 .. degreeN sigmaN]\n");
  HELP_APPEND(
      "                     : ngauss = number of gaussians which compose "
      "kernel (%d)\n",
      ngauss);
  HELP_APPEND(
      "                     : degree = degree of polynomial associated with "
      "gaussian #\n");
  HELP_APPEND("                                (%d %d %d)\n", D_DEG_GAUSS1,
              D_DEG_GAUSS2, D_DEG_GAUSS3);
  ...
  HELP_APPEND("                     : (%d %d %.2f %d %.2f %d %.2f\n", ngauss,
              D_DEG_GAUSS1, D_SIG_GAUSS1, D_DEG_GAUSS2, D_SIG_GAUSS2,
              D_DEG_GAUSS3, D_SIG_GAUSS3);

  HELP_APPEND(
      "   [-nt numthread]   : number of threads to use (%d) - this will be "
      "limited to nrx*nry\n\n",
      D_NTHREAD);
  ...
  HELP_APPEND(
      "       PHOTNORM            (to which system the difference image is "
      "normalized)\n");
  ...
  HELP_APPEND(
      "       REGION??            (IRAF-format limits for each region in the "
      "image)\n");
  HELP_APPEND(
      "       CONVOL??            (which image was convolved for each "
      "region)\n");
  HELP_APPEND(
      "       KSUM??              (sum of the convolution kernel for each "
      "region)");
#undef HELP_APPEND
  ...
  */
  ...
  if ((fits_is_reentrant() != 1) && nThread > 1) {
    fprintf(stderr,
            "FATAL ERROR nt (%d) : number of threads > 1 and cfitsio not "
            "compiled to be thread-safe (requires the `--enable-reentrant` "
            "argument on configuration)\n",
            nThread);
    exit(1);
  }
  ...
  if (nThread > nRegX * nRegY) {
    nThread = nRegX * nRegY;
    fprintf(stderr,
            "Limiting number of threads to equal the number of regions - set "
            "nt to %d\n",
            nThread);
  }
  ...
  if ((scaleFitThresh > 1) || (scaleFitThresh < 0)) {
    fprintf(stderr,
            "FATAL ERROR sft (%f) : cannot be negative or greater than 1\n",
            scaleFitThresh);
    exit(1);
  }
  if ((minFracGoodStamps > 1) || (minFracGoodStamps < 0)) {
    fprintf(stderr,
            "FATAL ERROR nft (%f) : cannot be negative or greater than 1\n",
            minFracGoodStamps);
    exit(1);
  }
  if (kerSigReject < 0) {
  ...
  if (forceConvolve && (!((strncmp(forceConvolve, "i", 1) == 0) ||
                          (strncmp(forceConvolve, "t", 1) == 0) ||
                          (strncmp(forceConvolve, "b", 1) == 0)))) {
    fprintf(stderr, "FATAL ERROR c (%s) : invalid char ('i' or 't' or 'b')\n",
            forceConvolve);
    exit(1);
  }
  if (photNormalize && (!((strncmp(photNormalize, "i", 1) == 0) ||
  ...
  if (figMerit &&
      (!((strncmp(figMerit, "s", 1) == 0) || (strncmp(figMerit, "h", 1) == 0) ||
         (strncmp(figMerit, "v", 1) == 0)))) {
